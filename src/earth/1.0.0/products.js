...
var products = (function () {
    ...
    wind: {
      ...
      create: function (attr) {
        return buildProduct({
          ...
          particles: { velocityScale: 1 / 100000, maxIntensity: 17 },
        });
      },
    },
      ...
      create: function (attr) {
        return buildProduct({
          ...
          scale: {
            ...
            // bounds: [273.15, 313.15],
            // gradient: µ.segmentedColorScale([
            //   [273.15, [21, 84, 187]], // 0 C
            //   [283.15, [247, 251, 59]], // 10 C
            //   [293.15, [235, 167, 21]], // 20 C
            //   [303.15, [230, 71, 39]], // 30 C
            //   [313.15, [88, 27, 67]], // 40 C
            // ]),
          },
        });
      },
   ...
   *           "parameterCategory": 2,
   *           "parameterNumber": 2,
   *           "surface1Type": 100,     // 기압면
   *           "surface1Value": 100000.0,       // 표면 등압면
   *           "forecastTime": 6,       //+6시간 예보
   *           "scanMode": 0,       // 북->남, 서->동 순서
   *           "nx": 360,
   *           "ny": 181,
   *           "lo1": 0,
   *           "la1": 90,
  ...
  function buildGrid(builder) {
    ...
    // 현재는 소장님 좌표계
    const [Δλ, Δφ] = [0.08261565864086151, 0.10223732143640518]; //소장님 github 기준 dx, dy
    ...
    return {
      ...
      forEachPoint: function (cb) {
        // grid=on일 때, grid의 인덱스(i,j)를 경도(λ), 위도(φ)로 변환
        for (var j = 0; j < nj; j++) {
          var row = grid[j] || [];
          for (var i = 0; i < ni; i++) {
            // 위도 계산
            // la1 = 첫줄 위도, la2 = 마지막줄 위도, Δφ = 한 줄 간 위도 간격(보통 양수)
            // 이 조건문이 데이터가 북→남 저장인지, 남→북 저장인지 자동 감지
            var φ;
            ...
            cb(µ.floorMod(180 + λ0 + i * Δλ, 360) - 180, φ, row[i]);
